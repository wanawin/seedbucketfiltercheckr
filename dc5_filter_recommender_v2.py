...
# ========================
# Constants / maps (aligned with tester)
# ========================
...
# aliases the CSVs sometimes expect
...
def sum_category_both(total: int):
    # Return both spellings to satisfy either convention in filters
    if 0 <= total <= 15:
...
def features_dict(seed_digits, prev_digits, prev2_digits):
    ...
    return {
        "sum_cat": sc_text,          # 'Very Low' etc (tester style)
        "sum_cat_compact": sc_compact, # 'VeryLow' etc (older style)
...
# ========================
# Step-0 generator (pair-from-seed + any 3, max digit count 2)
# ========================
...
# ========================
# Build expression context — now mirroring tester names
# ========================
def build_context_vars(seed_row, follower_mat, carry_rates, combo_digits):
    ...
    # carry & unions
    c2 = top2_carry_candidates(prev, carry_rates)                 # top-2 carry
    seedp1_set = sorted({(d+1) % 10 for d in seed})               # seed +1 (set)
    union2 = sorted(set(c2) | set(seedp1_set))                    # carry2 ∪ seed+1
    ...
    loc = {
        ...
        "prev_pattern": (sc_text, ("Even" if seed_sum %2==0 else "Odd")),  # minimal parity bundle
        ...
        "combo_sum_cat": sc_text,  # 'Very Low' etc
        ...
        # letter-map placeholders (so expressions referencing them parse)
        "digit_prev_letters": {}, "digit_current_letters": {},
        "prev_core_letters": set(), "core_letters_prevmap": [],

        # function aliases sometimes used in CSVs
...
def evaluate_filter_on_combo(expr, seed_row, follower_mat, carry_rates, combo_digits):
    try:
        ...
        # unquote whole expression if CSV stored it quoted
        if (expr.startswith('"') and expr.endswith('"')) or (expr.startswith("'") and expr.endswith("'")):
            expr = eval(expr)
        out = safe_eval(expr, loc)
        # In this language, True => eliminate. Keep if not eliminated.
        return not bool(out)
    except Exception:
        # If any error, do NOT eliminate (safe default = keep winner)
        return True
...
# Streamlit UI
# ========================
st.set_page_config(page_title="DC-5 Seed-Aware Filter Recommender v5", layout="wide")
st.title("DC-5 Seed-Aware Filter Recommender — Tester-Compatible (v5)")
...
if run_btn:
    if not hist_file or not filt_file:
        ...
        for i in range(len(results)-1):
            ...
            # rolling window (hot/cold); plus 10-draw hot7
        ...
        filt_df = filt_df[filt_df["expression"].str.len()>0 & filt_df["enabled"]]
        ...
        rcur = rows[cur]
        # bucket key from features (tester-compatible fields included)
        def bkey(row):
        ...
        with c1:
            ...
            st.write("Hot7 (10):", rcur.get("hot7_last10", []))
        with c2:
        ...
        if len(rec_df)==0:
            ...
            for feat in feat_cols:
                for i in prior:
                    ...
                    agg[(feat, val)]["n"] += 1
                    agg[(feat, val)]["k"] += int(keep_val==1)

            rows_out = []
            for (feat, val), d in agg.items():
                lb = wilson_lower_bound(d["k"], d["n"])
                rows_out.append({
        ...
        st.download_button("Download recommendations CSV", data=csv_buf.getvalue(), file_name="filter_recommendations_v5.csv", mime="text/csv")
