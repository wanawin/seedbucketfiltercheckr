...
st.set_page_config(page_title="DC-5 Seed-Aware Recommender — Manual Seed (v6-fast)", layout="wide")
st.title("DC-5 Seed-Aware Filter Recommender — Manual Seed (v6-fast)")
...
if run_btn:
    ...
    # Precompute "keep" per row per filter with progress
    st.info(f"Scoring {len(filt_df)} filters across {len(rows)} seeds…")
    progress = st.progress(0.0, text="Evaluating filters on historical winners…")
    per_index_keep = defaultdict(dict)
    total = len(rows) * len(filt_df)
    done = 0
    for i, r in enumerate(rows):
        for _, f in filt_df.iterrows():
            ...
            if done % 1000 == 0:
                progress.progress(min(1.0, done/total), text=f"Evaluated {done}/{total}…")
    ...
    pool_eval = pool if len(pool) <= max_pool_eval else random.sample(pool, int(max_pool_eval))
    ...
    # Diagnostics table stays the same but uses reduced rows to keep speed.
    ...
    st.download_button("Download recommendations CSV", data=csv_buf.getvalue(), file_name="filter_recommendations_v6_fast.csv", mime="text/csv")
